...
public class PlaceDenseLavaFloeProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		double raytrace_x = 0;
		double raytrace_z = 0;
		if ((entity instanceof LivingEntity _entity) ? _entity.isHolding(EvenbetternetherModBlocks.DENSE_LAVA_FLOE.get().asItem()) : false) {
			if (entity.level().clip(new ClipContext(entity.getEyePosition(1f), entity.getEyePosition(1f).add(entity.getViewVector(1f).scale(5)), ClipContext.Block.OUTLINE, ClipContext.Fluid.SOURCE_ONLY, entity)).getType() == HitResult.Type.BLOCK
				...
				if ((world.getFluidState(BlockPos.containing(raytrace_x, raytrace_y, raytrace_z)).createLegacyBlock()).getBlock() == Blocks.LAVA && world.isEmptyBlock(BlockPos.containing(raytrace_x, raytrace_y + 1, raytrace_z))) {
					world.setBlock(BlockPos.containing(raytrace_x, raytrace_y + 1, raytrace_z), EvenbetternetherModBlocks.DENSE_LAVA_FLOE.get().defaultBlockState(), 3);
					if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == EvenbetternetherModBlocks.DENSE_LAVA_FLOE.get().asItem()) {
						if (!(new Object() {
							public boolean checkGamemode(Entity _ent) {
								if (_ent instanceof ServerPlayer _serverPlayer) {
					...
					} else if ((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == EvenbetternetherModBlocks.DENSE_LAVA_FLOE.get().asItem()) {
						if (!(new Object() {
							public boolean checkGamemode(Entity _ent) {
								if (_ent instanceof ServerPlayer _serverPlayer) {
									return _serverPlayer.gameMode.getGameModeForPlayer() == GameType.CREATIVE;
